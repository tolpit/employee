...
/***/ function(module, exports, __webpack_require__) {
	...
	if(event&&event.request.url.startsWith("data:"))return;var request=event.request.clone();var response=new _response2.default(this,event);request.settings=this.settings;var stack=[];//Render when forced or when the user is offline
	if(this.get('force render')||!navigator.onLine){stack=this.middlewares.concat(_router2.default.match(request));}else{stack=[_middlewares2.default.NetworkFirst];}var index=0;while(index<stack.length){if(typeof stack[index]=="function")stack[index](request,response);index++;}}}]);return employee;}();employee.DB=_db2.default;employee.Store=_store2.default;employee.Router=_router2.default;for(var name in _middlewares2.default){employee[name]=_middlewares2.default[name];}self.employee=employee;

/***/ },
/* 1 */
/***/ function(module, exports, __webpack_require__) {

	"use strict";Object.defineProperty(exports,"__esModule",{value:true});var _debug=__webpack_require__(2);var _debug2=_interopRequireDefault(_debug);function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{default:obj};}var Router={routes:{}};Router.use=function(url,method,body,callbacks){var prepareURL=url;if(typeof prepareURL=="string"&&~prepareURL.indexOf(':')){prepareURL=Router.transform(prepareURL);}Router.routes[url]={originUrl:url,url:prepareURL,method:method,body:body,middlewares:callbacks};//Debug.log(Router.routes[url]);
...
/***/ function(module, exports) {
	...
	return this.endWell(res);}}],[{key:'set',value:function set(key,value){responseSettings[key]=value;}},{key:'get',value:function get(key,value){}}]);return res;}();exports.default=res;
...
/***/ function(module, exports) {
	...
	"use strict";Object.defineProperty(exports,"__esModule",{value:true});var middlewares={NetworkFirst:function NetworkFirst(req,res){// Debug.log("network", req);
	if(!navigator.onLine&&!res.notInCache){return middlewares.CacheFirst(req,res);}return fetch(req).then(function(response){var cacheCopy=response.clone();//Debug.log(response);
	if(response.type!=="opaque"&&req.method=="GET"){caches.open(req.settings.version+"::"+req.settings.name).then(function add(cache){cache.put(req,cacheCopy);//send it to the cache
	});}return res?res.network(response):response;},function(){middlewares.unableToResolve(req,res);}).catch(function(){middlewares.unableToResolve(req,res);});},CacheFirst:function CacheFirst(req,res){//Debug.log("cache", req);
	return caches.match(req).then(function(response){if(response)return res.cache(response);else{res.notInCache=true;return middlewares.NetworkFirst(req,res);}}).catch(function(err){console.trace(err);return middlewares.NetworkFirst(req,res);});},unableToResolve:function unableToResolve(req,res){return res.error();}};exports.default=middlewares;
