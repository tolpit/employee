...
/***/ function(module, exports, __webpack_require__) {
	...
	if(event&&event.request.url.startsWith("data:"))return;var request=event.request;var response=new _response2.default(this,event);request.settings=this.settings;var mws=[];//Render when forced or when the user is offline
...
/***/ function(module, exports) {
	...
	"use strict";Object.defineProperty(exports,"__esModule",{value:true});var middlewares={NetworkFirst:function NetworkFirst(req,res){Debug.log("network",req);if(!navigator.onLine)return middlewares.CacheFirst(req,res);var options={method:req.method,headers:req.headers};if(req.mode&&req.mode=="cors"){options.mode="cors";}Debug.log(options);return fetch(req,options).then(function(response){var cacheCopy=response.clone();caches.open(req.settings.version+"::"+req.settings.name).then(function add(cache){cache.put(req,cacheCopy);//send it to the cache
