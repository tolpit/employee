...
/***/ function(module, exports, __webpack_require__) {
	...
	var _innocent = __webpack_require__(7);
	
	var _innocent2 = _interopRequireDefault(_innocent);
	...
	var employee = function () {
	    function employee() {
	        ...
	        this.middlewares = [];
	        this.render = _innocent2.default;
	    ...
	    _createClass(employee, [{
	        ...
	        value: function handleInstall(event) {
	            ...
	            event.waitUntil(caches.open(this.cacheName()).then(function (cache) {
	                return cache.addAll(_this.get("cache"));
	            }));
	        }
	    }, {
	        key: "handleActivate",
	        value: function handleActivate(event) {
	            ...
	            event.waitUntil(caches.keys().then(function (keys) {
	                return Promise.all(keys.filter(function (key) {
	                    return !key.startsWith(_this2.get("version"));
	                }).map(function (key) {
	        ...
	        value: function handleFetch(event) {
	            ...
	            var response = new _response2.default(this, event);
	...
	employee.DB = _db2.default;
	employee.Store = _store2.default;
	employee.Router = _router2.default;
	
	for (var name in _middlewares2.default) {
	    employee[name] = _middlewares2.default[name];
...
/***/ function(module, exports, __webpack_require__) {
	...
	Router.delete = function (url, body) {
	    ...
	    _createClass(res, [{
	        ...
	        value: function prepare() {
	            ...
	            return new Promise(function (resolve, reject) {
	                _this2.endWell = resolve;
	                _this2.endBad = reject;
	            });
	        }
	    }, {
	        key: 'send',
	        ...
	        value: function render(name) {
	            ...
	            caches.match(this.app.get('views') + '/layout.html').then(function (response) {
	                return response.text();
	            }).then(function (layout) {
	                caches.match(_this3.app.get('views') + '/' + name + '.html').then(function (response) {
	                    _this3.headers = response.headers;
	                    _this3.headers.set("x-powered-by", "Employee");
	
	                    return response.text();
	                }).then(function (template) {
	                    _this3.send(_this3.app.render(layout, template, body));
	                });
	            });
	        }
	    }, {
	        key: 'network',
	        value: function network(res) {
	            return this.endWell(res);
	        }
	    }, {
	        key: 'cache',
	        value: function cache(res) {
	            return this.endWell(res);
	        }
	    }], [{
	        key: 'set',
...
/***/ function(module, exports) {
	...
	DB.connect = function (name, stores, callback) {
		if (!DB.indexedDB) console.log("Your browser doesn't support IndexedDB.");
		...
		if (DB.db) (callback || noop)({ init: true });
...
/***/ function(module, exports, __webpack_require__) {
	...
	function Store(name) {
		...
		["sort", "populate", "exec"].forEach(function (action) {
			self[action] = function (thing) {
				if (Array.isArray(self.actions[action])) self.actions[action].push(thing);else self.actions[action] = thing;
				return self;
			};
		});
	...
	Store.require = function (name) {
		...
		for (var i = 0; i < _db2.default.stores.length; i++) {
			if (_db2.default.stores[i] == name) idx = i;
		}
	
		_db2.default.stores[idx] = new Store(name);
		return _db2.default.stores[idx];
	};
	...
	Store.match = function (data, query) {
		...
		if (query._id && keys.length == 1) {
			//If "$in"
			if (query._id['$in']) {
				if (~query._id['$in'].indexOf(data._id)) return true;
			} else if (data._id == query._id) {
				return true;
			}
		}
	
		//Loop of verifications
		for (var key in query) {
			if (!pass || !entry[key] || entry[key] != query[key]) pass = false;
		}
	...
	Store.prototype.save = function (data, callback) {
		var transaction = _db2.default.db.transaction([this.name], "readwrite");
	...
	Store.prototype.find = function (query, fields, callback) {
		...
		var transaction = _db2.default.db.transaction([this.name], "readonly");
	...
	Store.prototype.doPopulate = function (storeData, callback) {
		...
		if (storeData === undefined) return callback(null);
	
		var field = self.actions.populate.shift();
		var ref = field;
		...
		var secondStore = Store.require(ref);
	
		if (Array.isArray(storeData)) {
			storeData.forEach(function (store, index) {
				var query = { "$in": !Array.isArray(store[field]) ? [store[field]] : store[field] };
	
				secondStore.find({ "_id": query }, function (res) {
					store[field] = !Array.isArray(store[field]) && res && res[0] ? res[0] : res;
	
					//last pass
					if (index == storeData.length - 1) callback(storeData);
				});
			});
		} else {
			var query = { "$in": !Array.isArray(storeData[field]) ? [storeData[field]] : storeData[field] };
	...
	Store.prototype.doExec = function (data) {
		...
		if (this.actions.populate.length > 0) {
...
/***/ function(module, exports) {
	...
	var middlewares = {
	    ...
	    OFFLINE: function OFFLINE(req, res) {},
	    ...
	    CacheFirst: function CacheFirst(req, res) {
	        return caches.match(req).then(function (response) {
	            if (response) return res.cache(response);else return middlewares.NetworkFirst(req, res);
	        }).catch(function (err) {
	            console.trace(err);
...
/***/ function(module, exports) {
	...
	var testInsert = new RegExp('#block\\s(.*)\\n?', 'mi');
	var testBlock = new RegExp('#block\\s(.*)\\n?([\\w\\n\\S\\s]*)#endblock', 'mi');
	var testIf = new RegExp('#if\\s(.*)\\n?([\\w\\n\\S\\s]*)#endif', 'mi');
	var testFor = new RegExp('#for\\s([a-z]+)\\s(in|of)\\s([a-z]+)\\n?([\\w\\n\\S\\s]*)#endfor', 'mi');
	...
	innocent.prepare = function prepare(layout, template) {
	    var layoutInsert = testInsert.exec(layout);
	    var templateBlock = testBlock.exec(template);
	
	    var _layoutInsert = _slicedToArray(layoutInsert, 2);
	
	    var layoutOrigin = _layoutInsert[0];
	    var layoutName = _layoutInsert[1];
	
	    var _templateBlock = _slicedToArray(templateBlock, 3);
	
	    var templateOrigin = _templateBlock[0];
	    var blockName = _templateBlock[1];
	    var content = _templateBlock[2];
	...
	innocent.step = function global(text, context) {
	    var matchs = text.match(test) || [];
	
	    matchs.map(function (key) {
	...
	innocent.if = function (text, context) {
	    var cond = testIf.exec(text);
	
	    if (!cond) return text;
	
	    var _cond = _slicedToArray(cond, 3);
	
	    var original = _cond[0];
	    var condition = _cond[1];
	    var content = _cond[2];
	
	
	    if (condition == condition.trim() && !context.hasOwnProperty(condition)) return text.replace(original, '');else return text.replace(original, content);
	};
	
	innocent.for = function (text, context) {
	    var loop = testFor.exec(text);
	
	    if (!loop) return text;else if (loop && loop.length < 5) return text.replace(loop[0], '');
	
	    var _loop = _slicedToArray(loop, 5);
	
	    var original = _loop[0];
	    var item = _loop[1];
	    var comparator = _loop[2];
	    var array = _loop[3];
	    var content = _loop[4];
	    ...
	    var forText = context[array].map(function (item) {
	        return innocent.step(content, item instanceof Object ? item : { item: item });
	    }).join("\n");
