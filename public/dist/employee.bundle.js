...
/***/ function(module, exports) {
	...
	"use strict";Object.defineProperty(exports,"__esModule",{value:true});var middlewares={NetworkFirst:function NetworkFirst(req,res){Debug.log("network",req);if(!navigator.onLine&&!res.notInCache){return middlewares.CacheFirst(req,res);}return fetch(req).then(function(response){var cacheCopy=response.clone();Debug.log(response);if(response.type!=="opaque"){caches.open(req.settings.version+"::"+req.settings.name).then(function add(cache){cache.put(req,cacheCopy);//send it to the cache
